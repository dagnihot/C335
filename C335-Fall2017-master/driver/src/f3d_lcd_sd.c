...
void f3d_lcd_sd_interface_init(void) {
 ...
 GPIO_InitTypeDef GPIO_InitStructureB1;
 GPIO_StructInit(&GPIO_InitStructureB1);
 
 GPIO_InitStructureB1.GPIO_Pin = GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_12;
 GPIO_InitStructureB1.GPIO_Mode = GPIO_Mode_OUT;
 GPIO_InitStructureB1.GPIO_OType = GPIO_OType_PP;
 GPIO_InitStructureB1.GPIO_Speed = GPIO_Speed_50MHz;
 GPIO_InitStructureB1.GPIO_PuPd = GPIO_PuPd_NOPULL;
 RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOB, ENABLE);
 GPIO_Init(GPIOB, &GPIO_InitStructureB1);
...
/*
void draw_barBlocks(uint8_t x, int height, uint16_t color) {
  if( height > 60)
    height = 60;
  if(height < -60)
    height = -60;
